...
import 'dart:io';
import 'dart:convert';
void main() => runApp(JsonParseSealsSts_SL());
class JsonParseSealsSts_SL extends StatefulWidget {

  JsonParseSealsSts_SL() : super();
...
class _JsonParseSealsState extends State <StatefulWidget> {
  ...
  final String url = global.urlVar +  '/seals_list'  + '?data_area=' + global.dataArea + '&status=No';
  ...
  Widget _buildUsersList() {
    return new ListView.builder(
      ...
      itemBuilder: (context, index) {
        return new Card(
          color: (_userDetails[index].used  == "No") ? Colors.redAccent : Colors.greenAccent,
  ...
  Widget _buildSearchResults() {
    return new ListView.builder(
      ...
      itemBuilder: (context, i) {
        return new Card(
          color: (_searchResult[i].used  == "Так") ? Colors.redAccent : Colors.greenAccent,
          margin: EdgeInsets.symmetric(vertical: 7),
          child: ListTile(
              title: Text(
